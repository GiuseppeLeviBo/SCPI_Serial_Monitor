...
49. **`lastres.csv` header creation on first store**
50. **`@store label` saves `last`**
51. **`@store label explicit_value` saves explicit value**
52. **`@startstore` saves every query**
53. **`@stopstore` disables autosave**
54. **`@comment` appends comment row**
55. **Newline sanitization (`\r\n` / `\r`)**
56. **Multiline CSV value quoting correctness**

### I) Debugger/UI workflow

57. **Normal run: no `step_event` blocking**
58. **Debug start: pause on first line**
59. **Single-step advances exactly one line**
60. **Pause↔Resume synchronization**
61. **Stop during pause unblocks thread and exits**
62. **Variable tab updates (`last`/global/local)**
63. **Current-line highlight on correct tab**
64. **No crash when script is not open in any tab**

### J) Error handling and resilience

65. **Runtime error log includes `script_name:L<line>`**
66. **After runtime error, `stop_requested=True` and run ends**
67. **`close_all()` tolerates disconnect exceptions**
